...
def save_telemetry_to_db(temp, rpm, vibration, status):
    """Inserts a real sensor reading into the SQL database."""
    with sqlite3.connect(DB_NAME) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO telemetry (temperature_c, rpm, vibration_level, status) VALUES (?, ?, ?, ?)",
            (temp, rpm, vibration, status)
        )
        conn.commit()
...
def get_historical_telemetry():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT timestamp, temperature_c, rpm, vibration_level, status FROM telemetry ORDER BY id DESC LIMIT 5")
...
digital_twin_brain = Agent(
    ...
    instructions=[
        ...
        "2. Analyze the data stream. It is considered an anomaly if:",
        "   - The most recent temperature exceeds 100°C",
        "   - OR the most recent vibration_level exceeds 0.85",
        "3. If an anomaly is found, call 'execute_emergency_shutdown' immediately.",
        "4. Provide a very brief, 2-line summary of your audit. Be direct and technical. If you triggered a shutdown, mention it in the summary."
    ],
    markdown=True
)
...
def run_digital_twin_simulation():
    ...
    while True:
        ...
        simulated_vibration = random.choice([0.15, 0.32, 0.45, 0.61, 0.95]) if current_status == "RUNNING" else 0.0
        
        # 1. Physical World: Save telemetry to the real SQL database
        save_telemetry_to_db(simulated_temp, simulated_rpm, simulated_vibration, current_status)
        print(f"\n[Sensors synced to DB] Temp: {simulated_temp}°C | RPM: {simulated_rpm} | Vibration: {simulated_vibration} | Status: {current_status}")
        ...
        # --- MODIFICA QUI: Stampiamo la risposta PRIMA di fare verifiche o break ---
        print(response.content)
        
        # 3. Controllo dello Shutdown: Intercettiamo la decisione dell'Agent o i limiti di sicurezza
        if "SHUTDOWN" in response.content.upper():
            current_status = "SHUTDOWN"
            print("\n[System Locked] Twin enforced shutdown. Terminating loop.")
            break
            
        # Aspetta 5 secondi prima del prossimo ciclo se tutto è normale
        time.sleep(5)
